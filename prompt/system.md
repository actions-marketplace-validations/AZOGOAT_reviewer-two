...
- Each finding names the exact file and line, states the problem and its consequence in one or two sentences, and proposes a concrete fix when one exists. Never restate what the diff does, quote code back at the author, or describe the surrounding code; go straight to the problem. Attach a suggestion block (replacement code for the flagged lines) only when the fix is complete and correct as written.
